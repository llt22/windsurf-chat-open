...
function makeRequest(port, prompt, context, reply, retryCount = 0) {
  return new Promise((resolve) => {
    ...
    req.on('error', (e) => {
      ...
      if ((e.code === 'ECONNREFUSED' || e.code === 'ECONNRESET') && retryCount < 3) {
        ...
        setTimeout(() => {
          makeRequest(port, prompt, context, reply, retryCount + 1).then(resolve);
        }, 2000); // 2秒后重试
      } else {
...
async function main() {
  ...
  if (replyIdx !== -1 && replyIdx + 1 < args.length) {
    reply = args[replyIdx + 1];
    args.splice(replyIdx, 2);
  }
  prompt = args.join(' ') || '等待用户反馈';
  const port = readPort();

  const response = await makeRequest(port, prompt, context, reply);
