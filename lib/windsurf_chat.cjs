...
function makeRequest(port, prompt, context, retryCount = 0) {
  return new Promise((resolve) => {
    ...
    req.on('error', (e) => {
      ...
      if ((e.code === 'ECONNREFUSED' || e.code === 'ECONNRESET') && retryCount < 3) {
        ...
        setTimeout(() => {
          makeRequest(port, prompt, context, retryCount + 1).then(resolve);
        }, 2000); // 2秒后重试
      } else {
...
async function main() {
  ...
  const response = await makeRequest(port, prompt, context);
